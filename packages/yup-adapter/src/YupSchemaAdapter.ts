  ...
  canHandle(schema: AnySchema): boolean {
    ...
    if (typeof s.describe === 'function') {
      try {
        ...
        return (
          typeof desc === 'object' && desc !== null && 'type' in (desc as Record<string, unknown>)
        );
      } catch {
        return false;
      }
        ...
        if (tests) {
          for (const test of tests) {
            switch (test.name) {
              case 'min': {
                const p = test.params;
                if (p) result.minLength = p.min ?? p.limit;
                break;
              }
              case 'max': {
                const p = test.params;
                if (p) result.maxLength = p.max ?? p.limit;
                break;
              }
              case 'length': {
                const p = test.params;
                if (p) {
                  const v = p.length ?? p.limit;
                  result.minLength = result.maxLength = v;
                }
                break;
              }
              case 'matches': {
                const p = test.params;
                if (p) result.pattern = p.regex ? String(p.regex) : p.pattern;
                break;
              }
              case 'email':
                result.format = 'email';
          ...
          for (const test of tests) {
            switch (test.name) {
              case 'min': {
                const p = test.params;
                if (p) {
                  result.minimum = p.min ?? p.limit ?? p.more;
                  if (p.more != null) result.exclusiveMinimum = true;
                }
                break;
              }
              case 'max': {
                const p = test.params;
                if (p) {
                  result.maximum = p.max ?? p.limit ?? p.less;
                  if (p.less != null) result.exclusiveMaximum = true;
                }
                break;
              }
              case 'integer':
        ...
        if (tests) {
          for (const test of tests) {
            switch (test.name) {
              case 'min': {
                const p = test.params;
                if (p) result.minItems = p.min ?? p.limit;
                break;
              }
              case 'max': {
                const p = test.params;
                if (p) result.maxItems = p.max ?? p.limit;
                break;
              }
              case 'length': {
                const p = test.params;
                if (p) {
                  const v = p.length ?? p.limit;
                  result.minItems = result.maxItems = v;
                }
                break;
              }
            }
